...
export function Kits({ artists = [] }) {
  ...
  const load = useCallback(() => {
    ...
    api.readKit(id).then((res) => {
      if (!res.ok) { setMsg(res.error || "Could not read that."); return; }
      const k = res.kit;
  ...
  const save = async () => {
    setBusy(true);
    setMsg("");
    const res = await api.saveKit(id, kit);
    setBusy(false);
  ...
  const makeLink = async () => {
    ...
    if (!res.ok) { setMsg(res.error || "Could not make a link."); return; }
  ...
  /*
    A LIST OF { url, credit } EDITED AS TEXT.

    One line per photograph, "address | credit". A row of paired inputs would
    be tidier and would also mean six taps to add a photograph on a phone.
    Pasting six lines from a message is one action.
  */
  const asLines = (list, keys) =>
    (list || []).map((o) => keys.map((k) => o[k] || "").join(" | ")).join("\n");
  ...
  return (
    <div>
      <IndexBand items={[
        { label: "ARTIST", value: (artists.find((a) => String(a.id) === String(id))?.name || "—").toUpperCase() },
        { label: "LIVE LINKS", value: String(links.filter((l) => !l.revoked).length).padStart(2, "0") },
        { label: "OPENED", value: String(links.reduce((n, l) => n + (l.uses || 0), 0)).padStart(3, "0") },
      ]} />
      ...
      {kit && (
        <>
          <Panel title="THE LINK YOU SEND">
            <p className="m-0 mb-4" style={{ ...fontText, fontSize: "16px", lineHeight: 1.55, color: theme.ink2 }}>
              This is what goes to a promoter. It does not expire — it gets
              revoked, which is a decision you make rather than a surprise on
              somebody else's Tuesday. Make a second one if you want to know
              which of two promoters passed it on.
            </p>

            {made && (
              <p className="m-0 mb-4 px-3 py-3" style={{
                ...fontUtility, fontSize: "12px", wordBreak: "break-all", color: theme.ink,
                background: theme.sunk, border: `1px solid ${theme.ink}`,
              }}>{made}</p>
            )}

            <Btn wide on onClick={makeLink}>MAKE A LINK</Btn>

            {links.length > 0 && (
              <div className="mt-5" style={{ borderTop: `1px solid ${theme.rule}` }}>
                {links.map((l) => (
                  <div key={l.token} className="flex items-baseline gap-3 py-2.5"
                       style={{ borderBottom: `1px solid ${theme.rule}`, opacity: l.revoked ? 0.45 : 1 }}>
                    <span className="flex-1 min-w-0 truncate" style={{ ...fontUtility, fontSize: "10px", color: theme.ink2 }}>
                      /kit/{l.token.slice(0, 10)}…
                    </span>
                    <span style={{ ...fontUtility, fontSize: "9px", letterSpacing: "0.14em", color: theme.ink2 }}>
                      {l.uses || 0} OPENED
                    </span>
                    {!l.revoked && <Btn danger onClick={() => kill(l.token)}>REVOKE</Btn>}
                    {!!l.revoked && (
                      <span style={{ ...fontUtility, fontSize: "9px", letterSpacing: "0.14em", color: theme.bad }}>DEAD</span>
                    )}
                  </div>
                ))}
              </div>
          ...
          <Panel title="WHAT IS ON IT">
            <Field label="Short biography — for a flyer, about 40 words">
              <textarea rows={3} value={kit.bioShort}
                        onChange={(e) => setKit((k) => ({ ...k, bioShort: e.target.value }))}
                        style={{ ...inputStyle, width: "100%", resize: "vertical", lineHeight: 1.5 }} />
            </Field>
            <div className="mt-4">
              <Field label="Long biography — for a programme">
                <textarea rows={7} value={kit.bioLong}
                          onChange={(e) => setKit((k) => ({ ...k, bioLong: e.target.value }))}
                          style={{ ...inputStyle, width: "100%", resize: "vertical", lineHeight: 1.55 }} />
              </Field>
            </div>
            <div className="mt-4">
              <Field label="Technical rider">
                <textarea rows={8} value={kit.rider}
                          onChange={(e) => setKit((k) => ({ ...k, rider: e.target.value }))}
                          placeholder={"2 × CDJ-3000\n1 × DJM-A9\nBooth monitor\n…"}
                          style={{ ...inputStyle, width: "100%", resize: "vertical", lineHeight: 1.6 }} />
              </Field>
            </div>
            <div className="mt-4">
              <Field label="Hospitality">
                <textarea rows={4} value={kit.hospitality}
                          onChange={(e) => setKit((k) => ({ ...k, hospitality: e.target.value }))}
                          style={{ ...inputStyle, width: "100%", resize: "vertical", lineHeight: 1.55 }} />
              </Field>
            </div>

            <div className="mt-4">
              <Field label="Photographs — one per line: address | credit | label">
                <textarea rows={4} value={asLines(kit.photos, ["url", "credit", "label"])}
                          onChange={(e) => setKit((k) => ({ ...k, photos: fromLines(e.target.value, ["url", "credit", "label"]) }))}
                          style={{ ...inputStyle, width: "100%", resize: "vertical", lineHeight: 1.6, fontSize: "13px" }} />
              </Field>
            </div>
            <div className="mt-4">
              <Field label="Logo files — one per line: address | label">
                <textarea rows={3} value={asLines(kit.logos, ["url", "label"])}
                          onChange={(e) => setKit((k) => ({ ...k, logos: fromLines(e.target.value, ["url", "label"]) }))}
                          style={{ ...inputStyle, width: "100%", resize: "vertical", lineHeight: 1.6, fontSize: "13px" }} />
              </Field>
            </div>
            <div className="mt-4">
              <Field label="Listen and follow — one per line: label | address">
                <textarea rows={4} value={asLines(kit.links, ["label", "url"])}
                          onChange={(e) => setKit((k) => ({ ...k, links: fromLines(e.target.value, ["label", "url"]) }))}
                          style={{ ...inputStyle, width: "100%", resize: "vertical", lineHeight: 1.6, fontSize: "13px" }} />
              </Field>
            </div>
            <div className="mt-4">
              <Field label="Who to write to about this artist">
                <input value={kit.contact} onChange={(e) => setKit((k) => ({ ...k, contact: e.target.value }))}
